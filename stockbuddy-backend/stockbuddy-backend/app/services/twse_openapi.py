...
class TWSEOpenAPI:
    ...
    # 不同資料的快取時間（秒）
    CACHE_TTL = {
        "per_dividend": 300,      # 本益比/殖利率: 5 分鐘
        "daily_trading": 60,      # 每日成交: 1 分鐘
        "market_index": 60,       # 大盤指數: 1 分鐘
        "institutional": 300,     # 三大法人: 5 分鐘
        "margin": 300,            # 融資融券: 5 分鐘
        "realtime": 15,           # 即時報價: 15 秒
    }
    ...
    def _get_cache(cls, key: str, cache_type: str = "default") -> Optional[Any]:
        ...
        if key in cls._cache:
            ttl = cls.CACHE_TTL.get(cache_type, 60)
    ...
    async def get_per_dividend_all(cls) -> Dict[str, Dict]:
        ...
        if cached:
            print("📦 [TWSE OpenAPI] 使用本益比/殖利率快取")
            return cached
        ...
        try:
            async with httpx.AsyncClient(timeout=30, verify=False, headers=cls.HEADERS) as client:
                ...
                if result:
                    ...
                    print(f"✅ [TWSE OpenAPI] 本益比/殖利率: {len(result)} 檔")
    ...
    async def get_daily_trading_all(cls) -> Dict[str, Dict]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30, verify=False, headers=cls.HEADERS) as client:
                ...
                if result:
                    cls._set_cache(cache_key, result)
                    print(f"✅ [TWSE OpenAPI] 每日成交: {len(result)} 檔")
    ...
    async def get_institutional_trading(cls, date: str = None) -> Dict[str, Dict]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30, verify=False, headers=cls.HEADERS) as client:
                ...
                if resp.status_code != 200:
                    print(f"⚠️ [TWSE] 三大法人 API 失敗: HTTP {resp.status_code}")
                    return {}
                
                data = resp.json()
                
                if "data" not in data:
                    print("⚠️ [TWSE] 三大法人: 無資料（可能非交易日）")
                    return {}
                ...
                if result:
                    cls._set_cache(cache_key, result)
                    print(f"✅ [TWSE] 三大法人: {len(result)} 檔")
                
                return result
                
        except Exception as e:
            print(f"❌ [TWSE] 三大法人錯誤: {e}")
            return {}
