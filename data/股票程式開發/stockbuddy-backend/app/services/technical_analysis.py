...
class TechnicalAnalysis:
    ...
    @staticmethod
    def calculate_kd(highs: List[float], lows: List[float], closes: List[float], period: int = 9) -> Dict[str, List[Optional[float]]]:
        """
        V10.17: 計算 KD 指標（隨機指標）
        ...
        for i in range(len(closes)):
            ...
            if i == period - 1:
                ...
                prev_k = k_values[-1] if k_values[-1] is not None else 50
            ...
            if i == period - 1:
                ...
                prev_d = d_values[-1] if d_values[-1] is not None else 50
                d = (2/3) * prev_d + (1/3) * k
    ...
    @staticmethod
    def analyze_kd(kd_data: Dict[str, List]) -> Dict[str, Any]:
        """V10.17: 分析 KD 指標"""
        k_values = kd_data["K"]
        d_values = kd_data["D"]

        if not k_values or k_values[-1] is None:
            return {"status": "資料不足", "K": None, "D": None, "score": 50, "signal": None}

        k = k_values[-1]
        d = d_values[-1]
        prev_k = k_values[-2] if len(k_values) > 1 and k_values[-2] is not None else k
        prev_d = d_values[-2] if len(d_values) > 1 and d_values[-2] is not None else d
        ...
        return {
            ...
            "status": status,
            "signal": signal,
            "score": int(min(100, max(0, base_score + cross_score))),
            "description": f"K值 {k:.1f}，D值 {d:.1f}，{status}，{signal}",
        }

    @staticmethod
    def analyze_williams_r(wr_values: List[float]) -> Dict[str, Any]:
        """V10.17: 分析威廉指標"""
        if not wr_values or wr_values[-1] is None:
            return {"status": "資料不足", "value": None, "score": 50}
        ...
        return {
            ...
            "status": status,
            "score": int(score),
            "description": f"威廉指標 {wr:.1f}%，{status}",
        }

    @staticmethod
    def calculate_risk_score(closes: List[float], volumes: List[float]) -> Dict[str, Any]:
        """
        V10.17: 計算風險評估分數
        ...
        """
        if len(closes) < 20:
            return {"risk_level": "未知", "risk_score": 50, "volatility": None}
        ...
        # 計算成交量波動
        volume_volatility = float(np.std(volumes[-20:]) / np.mean(volumes[-20:]) * 100) if np.mean(volumes[-20:]) > 0 else 50

        # 風險評分（波動越大風險越高）
        base_risk = volatility * 10  # 轉換為 0-100 尺度
        ...
        return {
            ...
            "description": f"波動率 {volatility:.2f}%，{risk_level}",
        }
    ...
    def full_analysis(cls, history_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # V10.17: 新增 KD、威廉指標和風險評估
        kd = cls.calculate_kd(highs, lows, closes)
        ...
        kd_analysis = cls.analyze_kd(kd)
        wr_analysis = cls.analyze_williams_r(williams_r)
        risk_analysis = cls.calculate_risk_score(closes, volumes)
        ...
        return {
            ...
            "rsi": rsi_analysis,
            "macd": macd_analysis,
            "kd": kd_analysis,  # V10.17: KD 指標
            "williams_r": wr_analysis,  # V10.17: 威廉指標
            "risk": risk_analysis,  # V10.17: 風險評估
            ...
            "summary": cls._generate_summary(trend_analysis, rsi_analysis, macd_analysis, volume_ratio, kd_analysis),
        }

    @staticmethod
    def _generate_summary(trend: Dict, rsi: Dict, macd: Dict, volume_ratio: float, kd: Dict = None) -> str:
        """V10.17: 生成技術分析摘要（含 KD 指標）"""
        parts = []
        ...
        # V10.17: KD 指標
        if kd and kd.get("signal"):
            parts.append(f"KD {kd['signal']}")
