...
class MarginService:
    ...
    def _is_cache_valid(cls, key: str) -> bool:
        ...
        if key not in cls._cache:
            return False
        last = cls._cache_time.get(key, 0)
    ...
    async def get_margin_data(cls, stock_id: str) -> Dict[str, Any]:
        ...
        優先使用 TWSE 真實 API，失敗時使用 FinMind，最後使用模擬資料
        """
        cache_key = f"margin_{stock_id}"
        ...
        try:
            ...
            if all_margin and stock_id in all_margin:
                data = all_margin[stock_id]
                result = cls._format_margin_data(data, "TWSE")
        ...
        try:
            ...
            if finmind_data and len(finmind_data) > 0:
                latest = finmind_data[-1]
                result = cls._format_finmind_data(latest, "FinMind")
                cls._cache[cache_key] = result
                cls._cache_time[cache_key] = datetime.now().timestamp()
                return result
        except Exception as e:
    ...
    @classmethod
    def _format_margin_data(cls, data: Dict, source: str) -> Dict[str, Any]:
        """格式化 TWSE 融資融券資料"""
        ...
        return {
            ...
            "comment": cls._get_margin_comment(margin_short_ratio, margin_change),
            "is_real_data": True,
            "data_source": source,
    ...
    @classmethod
    def _format_finmind_data(cls, data: Dict, source: str) -> Dict[str, Any]:
        """格式化 FinMind 融資融券資料"""
        # FinMind 欄位對應
        margin_buy = data.get("MarginPurchase", data.get("margin_buy", 0))
        margin_sell = data.get("MarginSale", data.get("margin_sell", 0))
        margin_balance = data.get("MarginBalance", data.get("margin_balance", 0))

        short_buy = data.get("ShortCovering", data.get("short_buy", 0))
        short_sell = data.get("ShortSale", data.get("short_sell", 0))
        short_balance = data.get("ShortBalance", data.get("short_balance", 0))
    ...
    @classmethod
    def _get_mock_data(cls, stock_id: str) -> Dict[str, Any]:
        """生成模擬資料 (最後備案)"""
        import random
        ...
        return {
            ...
            "is_real_data": False,
            "data_source": "mock",
            "note": "無法取得真實資料，使用模擬資料",
        }

    @classmethod
    def _get_margin_comment(cls, ratio: float, margin_change: int = 0) -> str:
        """
        V10.38: 融資融券評論
        ...
        if ratio > 30:
            ...
            comments.append("融資偏多，多方信心強")
        else:
            comments.append("融資主導，看多氣氛濃")

        # 融資變化評論
        if margin_change > 5000:
            comments.append("融資大增，散戶追進")
        elif margin_change > 1000:
            comments.append("融資增加")
        elif margin_change < -5000:
            comments.append("融資大減，散戶退場")
        elif margin_change < -1000:
            comments.append("融資減少")

        return "；".join(comments) if len(comments) > 1 else comments[0]
